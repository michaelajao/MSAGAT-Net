...
% ---------- TITLE & AUTHOR ----------
\title{\textbf{MSTGAT-Net: A Multi-Scale Temporal Graph Attention Network for Spatiotemporal Forecasting}}
...
The second core component of our MSAGAT-Net architecture is the Adaptive Graph Attention Module (AGAM). Traditional approaches to spatial modelling often rely on fixed adjacency matrices based on geographical proximity or administrative boundaries, which do not capture the evolving nature of epidemic spread influenced by factors such as population mobility, healthcare referral patterns, and socioeconomic connections. Based on the principles of graph attention networks \citep{velickovic2017graph}, our AGAM adaptively learns the relationships between regions based on their feature representations, rather than being constrained by a predefined graph structure. This adaptive approach allows the model to discover and leverage spatial dependencies that may not be immediately apparent from geographical proximity alone, and to adjust these dependencies as the epidemic evolves.
...
Recent advances in efficient attention mechanisms have shown that low-rank decomposition techniques can substantially reduce computational complexity whilst maintaining expressive power. Several influential works have explored this direction. Researchers such as \citet{puny2020global} propose a low-rank global attention (LRGA), an adaptive module that replaces the total attention of the dot product in GNN with a decomposed low-rank form. \citet{kong2023low} present the Global Representation Key (GRK) attention layer, where the attention scores of each node are calculated using a shared projection of the features of its neighbours. A learnt adaptive low-rank matrix captures the most salient structural information, mitigating over-smoothing and improving performance on graphs. While \citet{yang2023self} embeds an adaptive low-rank decomposition step in each propagation layer within each ego network to concentrate message passing on the most prominent low-dimensional subspaces. This lets the model adaptively focus on the most informative subspace per node, improving robustness without labels. These studies collectively demonstrate that low‑rank factorisation offers an efficient, scalable and expressive alternative to full‑rank attention in graph architectures, and motivate the design of this module in our framework.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\columnwidth]{figs/mtfm_diagram.pdf}
...
% \subsubsection{Advantages for Epidemic Forecasting}

% The MTFM offers several specific benefits for epidemic time-series forecasting:

% \begin{enumerate}
%     \item \textbf{Multi-scale awareness}: By explicitly modelling different temporal scales, the module can capture both rapid changes (such as weekend reporting effects) and longer-term trends (such as growth or decline phases of an epidemic).
    
%     \item \textbf{Computational efficiency}: The parallel processing of different scales is more efficient than sequential approaches like recurrent neural networks, allowing for faster training and inference.
    
%     \item \textbf{Adaptive importance}: The learnable fusion weights enable the model to adjust the importance of different temporal scales based on the specific characteristics of the epidemic data.
    
%     \item \textbf{Interpretability}: Analysis of the learned fusion weights can provide insights into which temporal scales are most informative for forecasting, potentially aiding epidemiological understanding.
% \end{enumerate}

% Through the MTFM, our model effectively captures the complex temporal dynamics of epidemic progression at multiple scales, complementing the spatial dependency modelling provided by the AGAM. The combined spatiotemporal representations are then passed to the final component of our architecture, the Progressive Multi-step Prediction Refinement Module, for generating accurate forecasts across multiple horizons.
...
% The PPRM component provides several specific benefits for epidemic forecasting applications:

% \begin{enumerate}
%     \item \textbf{Horizon-aware predictions}: By generating predictions for all horizons simultaneously and applying horizon-specific refinement, the module can adapt its strategy based on forecast distance.
    
%     \item \textbf{Incorporation of epidemiological patterns}: The exponential decay projection incorporates domain knowledge about common epidemic progression patterns, providing a valuable inductive bias.
    
%     \item \textbf{Robustness to anomalies}: The adaptive gating mechanism can de-emphasise the model prediction when recent observations suggest a different trajectory, improving robustness to unexpected changes in epidemic dynamics.
    
%     \item \textbf{Smooth transition from observations}: The refined predictions naturally transition from the most recent observations to model-based forecasts, avoiding discontinuities that could reduce forecast usability.
    
%     \item \textbf{Interpretable forecast decomposition}: The gate values provide insight into which regions and horizons rely more heavily on recent trends versus learnt patterns, potentially offering valuable epidemiological insights.
% \end{enumerate}

% Through the PPRM, our MSAGAT-Net architecture generates refined multi-horizon forecasts that balance model-based predictions with epidemiological progression tendencies, resulting in more accurate and reliable epidemic forecasts across different temporal horizons.
...
\begin{itemize}
    ...
    \item \textbf{NHS-ICUBeds Dataset:} Obtained from the National Health Service (NHS) England\citep{NHS2024HospitalActivity}, this dataset provides daily counts of occupied mechanical ventilator beds in seven regions of the NHS from March 2020 to February 2022. Unlike the other datasets that focus on case counts, this dataset offers an opportunity to evaluate the model's capability to predict healthcare resource utilisation, which is critical for effective epidemic response and management.
\end{itemize}
...
The training algorithm for the MSAGAT-Net model is formalised in Algorithm~\ref{alg:MSAGAT-Net_training}, which outlines the step-by-step procedure to optimise model parameters through backpropagation and early stopping.

\begin{algorithm}[h]
    \caption{MSAGAT-Net Training Algorithm}
    \label{alg:MSAGAT-Net_training}
    \KwIn{Data $\mathbf{X} \in \mathbb{R}^{N \times T}$, adjacency $\mathbf{A} \in \mathbb{R}^{N \times N}$, hyperparameters}
    \KwOut{Trained parameters $\mathbf{\Theta}^*$}
    
    Initialize model parameters, optimizer, scheduler\;
    $L_{\text{val}}^* \leftarrow \infty$, $p \leftarrow 0$\;
    
    \For{epoch $e = 1$ \KwTo $E_{\max}$}{
      \ForEach{batch $(\mathbf{X}_b, \mathbf{y}_b)$ in training set}{
        $\mathbf{F} \leftarrow \emptyset$\;
        \For{region $i \in \{1,\ldots,N\}$}{
        ...
        $\mathbf{\hat{y}}_b \leftarrow \text{PPRM}(\mathbf{H}, \mathbf{X}_b[:,-1,:])$\;
        
        $L_b \leftarrow \text{MSE}(\mathbf{\hat{y}}_b, \mathbf{y}_b) + \lambda_{\text{attn}} \cdot L_{\text{attn}}$\;
        Update parameters via backpropagation\;
      }
      
      Compute validation loss $L_{\text{val}}$\;
      Update scheduler\;
      
      \If{$L_{\text{val}} < L_{\text{val}}^*$}{
        $\mathbf{\Theta}^* \leftarrow \mathbf{\Theta}$, $L_{\text{val}}^* \leftarrow L_{\text{val}}$, $p \leftarrow 0$\;
      }
      \ElseIf{$p + 1 \geq P$}{
        \textbf{break}\;
      }
...
Comparison of MSAGAT-Net with the two strongest baselines, Cola-GNN and EpiGNN, reveals interesting patterns. In the dataset of Japan-Prefectures, MSAGAT-Net consistently outperforms both models across all metrics and horizons. This suggests that MSAGAT-Net's multi-scale module and adaptive graph attention mechanism provide significant advantages for this particular dataset, which features complex spatio-temporal patterns across Japan's diverse prefectures.
...
\begin{figure*}[htbp]
  ...
  \caption{Adjacencies and learnt attention matrices of MSAGAT-Net on the Japan-Prefectures dataset for 5-day horizon.}
...
\subsection{Ablation Study of Model Components}
\label{sec:ablation}

% To systematically evaluate the contribution of each architectural component to MSAGAT-Net's overall performance, we conducted comprehensive ablation studies across multiple datasets and forecast horizons. This analysis helps quantify the individual contribution of each component and validate our architectural design choices across different epidemic contexts. We focused our primary experiments on the Japan-Prefectures dataset with a fixed window size of 20 time steps and varying forecast horizons (3, 5, 10, and 15 days), representing short to long-term predictions. 

We performed in-depth ablation studies across various datasets and forecast horizons to systematically assess each architectural component's contribution to the overall performance of MSAGAT-Net.
This rigorous analysis provides valuable information on how each component affects the accuracy of forecasting in different epidemic contexts and time scales, helping to validate our architectural design choices and identify opportunities for context-specific optimisations.
...
\begin{itemize}
    \item \textbf{Without AGAM}: Replacing the Efficient Adaptive Graph Attention Module with a standard graph convolutional layer that lacks the adaptive attention mechanisms.
    
    \item \textbf{Without MTFM}: Replacing the Dilated Multi-scale Module with a single-scale temporal convolutional layer with fixed kernel size and dilation rate.
    
    \item \textbf{Without PPRM}: Replacing the Progressive Prediction Module with direct multi-step prediction without the adaptive refinement mechanism.
\end{itemize}

All model variants were trained using identical hyperparameters, optimisation procedures, and data splits to ensure fair comparison. We evaluated performance using four complementary metrics: Mean Absolute Error (MAE), Root Mean Square Error (RMSE), Pearson Correlation Coefficient (PCC) and the coefficient of determination (R²).

Tables~\ref{tab:ablation_japan} and \ref{tab:ablation_ltla} present the comprehensive results of our ablation experiments. The values in parentheses indicate the percentage change relative to the full model, with positive values for error metrics (MAE, RMSE) indicating performance degradation and negative values indicating performance improvement. For correlation metrics (PCC, R²), positive values indicate improvement and negative values indicate degradation.

\begin{table*}[htbp]
    \centering
    \caption{Ablation study results on Japan-Prefectures dataset with window size 20 across different forecast horizons. Values in parentheses indicate percentage change relative to the full model.}
    \label{tab:ablation_japan}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{llcccccc}
        ...
        & MAE  & 328.57  & (+1.30\%) & 388.46  & (-0.86\%) & 613.53  & (+32.77\%) \\
        ...
        & MAE  & 315.59  & (-2.70\%) & 364.63  & (-6.94\%) & 470.36  & (+1.79\%)  \\
        ...
        & MAE  & 348.91  & (+7.57\%)  & 339.21  & (-13.43\%) & 445.68  & (-3.55\%)  \\
...
\begin{figure*}[htbp]
  ...
  \begin{subfigure}[b]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{figs/rmse_only_japan_h3.pdf}
    \caption{RMSE comparison across model variants for the Japan dataset horizon 3}
    \label{fig:ablation:rmse_h3}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{figs/rmse_only_japan_h5.pdf}
    \caption{RMSE comparison across model variants for the Japan dataset horizon 5}
  ...
  \begin{subfigure}[b]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{figs/rmse_only_japan_h10.pdf}
    \caption{RMSE comparison across model variants for the Japan dataset horizon 10}
    \label{fig:ablation:rmse_h10}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{figs/rmse_only_japan_h15.pdf}
    \caption{RMSE comparison across model variants for the Japan dataset horizon 15}
    \label{fig:ablation:rmse_h15}
  \end{subfigure}
  \caption{Comparison of RMSE across different model variants for the Japan dataset.}
  \label{fig:ablation:rmse}
\end{figure*}

Our ablation study reveals striking differences in the importance of components between the influenza data sets (Japan-Prefectures) and COVID-19 (LTLA-Timeseries), as well as between different forecast horizons, as visualised in Figure~\ref{fig:ablation:rmse}.

\subsubsection{Impact of AGAM Removal}

The Adaptive Graph Attention Module shows dramatically different patterns of importance across the two datasets:

For the dataset of Japan-Prefectures, the removal of AGAM consistently degrades the performance of RMSE in the short term (3 days: +5. 28\%) and medium term forecasts (5 days: +2. 48\%), with a particularly dramatic impact for longer term forecasts (10 days: +23. 12\% RMSE, -22. 54\% PCC, -38. 14\% R2), as clearly visible in Figure~\ref{fig:ablation:rmse_h10}. This escalating pattern suggests that spatial dependencies become increasingly crucial for longer-term influenza forecasting, likely reflecting the complex spatial transmission dynamics of seasonal influenza across Japan's diverse prefectures.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\linewidth]{figs/matrices_MSTAGAT-Net.japan.w-20.h-5.no_agam.png}
  \caption{Adjacencies and learnt attention matrices of MSAGAT-Net on the Japan-Prefectures dataset for 5-day horizon with no agam module}
  \label{fig:attention_matrices_no_agam}
\end{figure}

The figure \ref{fig:attention_matrices_no_agam} illustrates the attention matrices learnt by MSAGAT-Net on the Japan-Prefectures dataset for a 5-day forecast horizon without the AGAM module. Also figure \ref{fig:rmse_vs_horizon} presents a comparison of RMSE values across different model variants (including the full MSAGAT-Net and its ablated versions) for selected datasets, illustrating how prediction error changes with increasing forecast horizons.


\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\linewidth]{figs/rmse_vs_horizon.pdf}
  \caption{Comparison of RMSE across different model variants for some choosen dataset}
...
\subsubsection{Impact of MTFM Removal}

The multi-scale Fusion Module shows surprisingly consistent effects across both datasets, with its contribution being minimal or even slightly negative in most scenarios:

For the dataset of Japan-Prefectures, the removal of MTFM leads to only slight changes in RMSE across all horizons (3-day: +1.57\%; 5-day: -0.87\%; 10-day: +0.68\%), with negligible impacts on correlation metrics, as evidenced in all panels of Figure~\ref{fig:ablation:rmse}. More notably, removal of MTFM actually improves MAE across multiple horizons (3-day: -2.70\%; 5-day: -6.94\%), suggesting that simpler temporal modelling might reduce absolute error in some cases. For the 15-day horizon (Figure~\ref{fig:ablation:rmse_h15}), removing MTFM appears to noticeably improve performance, suggesting that simpler temporal processing may be advantageous for very long-term forecasts.
...
\begin{figure*}[htbp]
  \centering
  \includegraphics[width=0.9\linewidth]{figs/matrices_MSTAGAT-Net.japan.w-20.h-5.no_mtfm.png}
  \caption{Adjacencies and learnt attention matrices of MSAGAT-Net on the Japan-Prefectures dataset for 5-day horizon with no mtfm module}
  \label{fig:attention_matrices_no_mtfm}
\end{figure*}

\subsubsection{Impact of PPRM Removal}

The Progressive Prediction Refinement Module reveals the most dramatic and divergent effects between the two datasets:

PPRM removal degrades performance for short-term forecasts (3-day: +2.81\% RMSE, +7.57\% MAE), as shown in Figure~\ref{fig:ablation:rmse_h3}, but surprisingly improves performance for medium- and longer-term horizons (5-day: -1.01\% RMSE, -13.43\% MAE; 10-day: -3.60\% RMSE, -3.55\% MAE), which is visible in Figures~\ref{fig:ablation:rmse_h5} and \ref{fig:ablation:rmse_h10}. In particular, PPRM removal consistently improves correlation metrics across all horizons (PCC: +2.15\%, +1.43\%, +2.95\%), suggesting that direct prediction better captures the underlying trends even when it occasionally increases the absolute error. This pattern indicates that progressive refinement helps with immediate influenza forecasts, but direct prediction becomes more effective for longer horizons in this context for the Japan datatset.
...
\begin{figure*}[htbp]
  \centering
  \includegraphics[width=0.9\linewidth]{figs/matrices_MSTAGAT-Net.japan.w-20.h-5.no_pprm.png}
  \caption{Adjacencies and learnt attention matrices of MSAGAT-Net on the Japan-Prefectures dataset for 5-day horizon with no pprm module}
  \label{fig:attention_matrices_no_pprm}
\end{figure*}

This striking divergence in the importance of PPRM probably reflects fundamental differences in the predictability and volatility of the influenza versus COVID-19 transmission dynamics. Seasonal influenza follows more established patterns that might be amenable to direct prediction, especially for medium and longer horizons, while COVID-19 transmission during the study period was characterised by rapid changes due to policy interventions and emerging variants, necessitating a more adaptive progressive refinement approach.

A particularly intriguing finding from our ablation study is the non-linear relationship between component importance and forecast horizon. Rather than the importance of the components increasing or decreasing monotonically with the length of the horizon, we observe complex patterns that challenge simplistic assumptions, as clearly illustrated in the different panels of Figure~\ref{fig:ablation:rmse}. The figure 

For the dataset of Japan-Prefectures, the importance of AGAM increases dramatically from 5-day (+2.48\% RMSE when removed) to 10-day (+23.12\%), suggesting a critical threshold where spatial modelling becomes essential. However, data from the 15-day horizon (Figure~\ref{fig:ablation:rmse_h15}) reveal that the importance of AGAM unexpectedly decreases again (+0.98\% RMSE), indicating a non-monotonic relationship. Similarly, MTFM removal becomes increasingly beneficial as the horizon extends to 15 days (-3.70\% RMSE), contrary to the typical assumption that temporal modelling becomes more important for longer horizons.
...
\begin{figure*}[ht]
  ...
  \begin{subfigure}[b]{0.48\linewidth}
    ...
    \caption{component impact for Japan dataset horizon 3}
  ...
  \begin{subfigure}[b]{0.48\linewidth}
    ...
    \caption{component impact for Japan dataset horizon 5}
  ...
  \begin{subfigure}[b]{0.48\linewidth}
    ...
    \caption{component impact for Japan dataset horizon 10}
  ...
  \begin{subfigure}[b]{0.48\linewidth}
    ...
    \caption{component impact for Japan dataset horizon 15}
    \label{fig:component_impact_japan_h15}
  \end{subfigure}
  \caption{Comparison of component impact across different model variants for the Japan dataset.}
  \label{fig:component_impact_japan}
\end{figure*}

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.9\linewidth]{figs/component_importance_heatmap.pdf}
  \caption{Component importance heatmap across different datasets and forecast horizons.}
  \label{fig:component_importance_heatmap}
\end{figure}


The component importance heatmap in Figure~\ref{fig:component_importance_heatmap} summarises the relative importance of each component in all datasets and forecast horizons. The heatmap shows the percentage importance of the component ablation study of the japan influeza dataset.
