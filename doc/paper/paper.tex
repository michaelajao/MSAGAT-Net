...
% ---------- TITLE & AUTHOR ----------
\title{\textbf{MSAGAT-Net: A Multi-Scale Temporal Graph Attention Network for Spatiotemporal Epidemic Forecasting}}
...
Accurate epidemic forecasting across multiple geographical regions is crucial for effective public health response and resource allocation. The COVID-19 pandemic has dramatically highlighted the importance of reliable predictive models that can capture both the spatial spread of diseases across interconnected regions and the temporal evolution of transmission dynamics. However, developing such models presents significant challenges due to the complex interplay between spatial dependencies, temporal patterns, and the inherent uncertainty in disease transmission.
...
In this paper, we introduce MSAGAT-Net (Multi-Scale Temporal Graph Attention Network), a novel deep learning architecture that addresses these limitations through three key innovations:

\begin{itemize}
\item An Efficient Adaptive Graph Attention Module (EAGAM) that captures spatial dependencies with linear-time complexity O(N) instead of the quadratic O($N^2$) complexity of traditional attention mechanisms
\item A Dilated Multi-Scale Temporal Module (DMTM) that processes temporal patterns at multiple resolutions to capture both immediate changes and gradual trends
\item A Progressive Prediction Module (PPM) that enables accurate multi-horizon forecasting through iterative refinement
\end{itemize}
...
\caption{Data flow through the Multi-scale Temporal Feature Module (MTFM) architecture, showing parallel processing at multiple dilation scales and adaptive fusion of temporal patterns.}
...
The PPRM addresses the critical issue of error accumulation inherent in multi-horizon epidemiological forecasting. Through an extensive analysis of multistep forecasting failures, it became evident that prediction errors compound exponentially with increasing prediction horizons \cite{BENTAIEB20127067, chandra2021evaluation}. This module incorporates concepts from residual error correction, ensemble learning, and adaptive gating mechanisms common in recurrent neural network architectures \cite{hochreiter1997long}.
...
\begin{figure*}[htbp]
  \centering
  % Subfigure replaced with subfloat for IEEEtran compatibility
  \subfloat[RMSE comparison across model variants for the Japan dataset horizon 3]{
    \includegraphics[width=0.45\linewidth]{figs/rmse_only_japan_h3.pdf}
    \label{fig:ablation:rmse_h3}
  }
  \hfill
  % Subfigure replaced with subfloat for IEEEtran compatibility
  \subfloat[RMSE comparison across model variants for the Japan dataset horizon 5]{
    \includegraphics[width=0.45\linewidth]{figs/rmse_only_japan_h5.pdf}
  ...
  \hfill
  % Subfigure replaced with subfloat for IEEEtran compatibility
  \subfloat[RMSE comparison across model variants for the Japan dataset horizon 10]{
    \includegraphics[width=0.45\linewidth]{figs/rmse_only_japan_h10.pdf}
    \label{fig:ablation:rmse_h10}
  }
  \hfill
  % Subfigure replaced with subfloat for IEEEtran compatibility
  \subfloat[RMSE comparison across model variants for the Japan dataset horizon 15]{
    \includegraphics[width=0.45\linewidth]{figs/rmse_only_japan_h15.pdf}
    \label{fig:ablation:rmse_h15}
  }
  \caption{Ablation study results showing RMSE comparison across different MSAGAT-Net model variants for the Japan-Prefectures dataset at forecast horizons of 3, 5, 10, and 15 days, demonstrating the contribution of each architectural component.}
...
\begin{figure}[htbp]
  ...
  \caption{Attention matrices for MSAGAT-Net on the Japan-Prefectures dataset with 5-day forecast horizon when the AGAM module is removed, showing degraded spatial dependency learning.}
...
\begin{figure}[htbp]
  ...
  \caption{RMSE performance comparison across different MSAGAT-Net model variants as a function of forecast horizon, showing how component importance varies with prediction time scale.}
...
\begin{figure*}[htbp]
  ...
  \caption{Attention matrices for MSAGAT-Net on the Japan-Prefectures dataset with 5-day forecast horizon when the MTFM module is removed, demonstrating the impact of multi-scale temporal processing on spatial attention patterns.}
...
\begin{figure*}[htbp]
  ...
  \caption{Attention matrices for MSAGAT-Net on the Japan-Prefectures dataset with 5-day forecast horizon when the PPRM module is removed, showing how progressive prediction refinement affects learned spatial dependencies.}
...
\begin{figure*}[ht]
  \centering
  % Subfigure replaced with subfloat for IEEEtran compatibility
  \subfloat[Component impact for Japan dataset horizon 3]{
    \includegraphics[width=0.45\linewidth]{figs/component_impact_japan_h3.pdf}
    \label{fig:component_impact_japan_h3}
  }
  \hfill
  % Subfigure replaced with subfloat for IEEEtran compatibility
  \subfloat[Component impact for Japan dataset horizon 5]{
    \includegraphics[width=0.45\linewidth]{figs/component_impact_japan_h5.pdf}
    \label{fig:component_impact_japan_h5}
  }
  \hfill
  % Subfigure replaced with subfloat for IEEEtran compatibility
  \subfloat[Component impact for Japan dataset horizon 10]{
    \includegraphics[width=0.45\linewidth]{figs/component_impact_japan_h10.pdf}
    \label{fig:component_impact_japan_h10}
  }
  \hfill
  % Subfigure replaced with subfloat for IEEEtran compatibility
  \subfloat[Component impact for Japan dataset horizon 15]{
    \includegraphics[width=0.45\linewidth]{figs/component_impact_japan_h15.pdf}
    \label{fig:component_impact_japan_h15}
  }
  \caption{Component ablation impact analysis for MSAGAT-Net on the Japan-Prefectures dataset, showing percentage change in RMSE when individual components (AGAM, MTFM, PPRM) are removed across forecast horizons of 3, 5, 10, and 15 days.}
...
\begin{figure}[ht]
  ...
  \caption{Component importance heatmap showing relative performance impact (percentage change in RMSE) when individual architectural components are removed, across different epidemic datasets and forecast horizons.}
...
Future work will explore automatic architecture search for horizon-specific optimization, incorporation of external factors such as mobility data and policy interventions, and extension to multi-variate forecasting including hospitalizations and resource utilization. The efficiency and interpretability of MSAGAT-Net make it a promising tool for real-time epidemic surveillance and public health decision support.
