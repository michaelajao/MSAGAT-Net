...
% ---------- ABSTRACT ----------
\begin{abstract}
Spatiotemporal forecasting presents significant challenges in machine learning research, requiring models that can simultaneously capture complex spatial relationships and temporal dynamics. This problem is particularly crucial in domains such as epidemiology, where disease spread exhibits intricate patterns across both geographical regions and time periods. We present MSTGAT-Net (Multi-Scale Temporal Graph Attention Network), a novel deep learning architecture that combines adaptive graph attention mechanisms with autoregressive prediction. The model incorporates three key components: (1) a Temporal Feature Extraction Module (TFEM) that captures temporal patterns from input time series using efficient depthwise separable convolutions, (2) a Low-rank Adaptive Graph Attention Module (LR-AGAM) that dynamically captures evolving spatial relationships between regions using efficient low-rank approximations, and (3) a GRU-based Progressive Prediction Refinement Module (PPRM) that generates multi-step forecasts autoregressively while adaptively blending with recent observations to mitigate error accumulation. Our comprehensive ablation studies demonstrate that each component significantly contributes to the overall forecasting performance. MSTGAT-Net offers robust performance across diverse forecasting horizons while maintaining computational efficiency, making it particularly valuable for epidemiological forecasting and healthcare resource planning applications.
\end{abstract}
...
In this paper, we introduce the Multi-Scale Temporal Graph Attention Network (MSTGAT-Net), a novel deep learning architecture specifically designed to address these limitations. MSTGAT-Net integrates three innovative components: (1) temporal feature extraction using efficient depthwise separable convolutions along the time axis, (2) adaptive graph learning through a low-rank attention mechanism for spatial dependencies, and (3) GRU-based autoregressive horizon prediction with adaptive refinement.

Our contributions can be summarized as follows:
\begin{itemize}
    \item We propose a streamlined end-to-end architecture that processes temporal patterns through convolutional feature extraction, models spatial dependencies through attention, and generates forecasts through autoregressive prediction.
    \item We introduce a Temporal Feature Extraction Module (TFEM) using depthwise separable convolutions that efficiently encodes temporal dynamics from each node's historical observations.
    \item We develop a Low-rank Adaptive Graph Attention Module (LR-AGAM) that learns spatial relationships during training without relying on a predefined adjacency matrix, using efficient low-rank approximations.
    \item We design a GRU-based Progressive Prediction Refinement Module (PPRM) that generates multi-step forecasts autoregressively while adaptively blending with learned decay extrapolations, improving forecasting accuracy particularly during regime changes.
...
Our proposed MSTGAT-Net addresses these limitations through a carefully designed architecture that combines adaptive graph attention with multi-scale spatial feature fusion and progressive prediction refinement. This integrated approach achieves state-of-the-art performance while maintaining computational efficiency.
...
\begin{figure}[ht]
\centering
\begin{tikzpicture}[node distance=1.8cm and 3cm, >=Latex, 
    ...
    \node (feat_ext) [module, below=of input] {TFEM\\(Temporal Features)};
    \node (spatial) [module, below=of feat_ext] {LR-AGAM\\(Spatial Attention)};
    \node (predict) [module, below=of spatial] {PPRM (GRU)\\(Autoregressive Pred.)};
    \node (output) [output, below=of predict] {Output $\hat{\mathbf{Y}}$};
    
    % Main flow
    \draw [->, thick] (input) -- (feat_ext);
    \draw [->, thick] (feat_ext) -- (spatial);
    \draw [->, thick] (spatial) -- (predict);
    \draw [->, thick] (predict) -- (output);
    
    % Skip connection from TFEM to LR-AGAM
    ...
    % Input to Prediction Module (for refinement with last observation)
...
\end{tikzpicture}
\caption{Overall architecture of the proposed MSTGAT-Net model. The framework consists of three key components: Temporal Feature Extraction Module (TFEM), Low-rank Adaptive Graph Attention Module (LR-AGAM), and GRU-based Progressive Prediction Refinement Module (PPRM). Dashed lines indicate skip connections and the direct connection from input to PPRM for adaptive refinement.}
\label{fig:architecture}
\end{figure}

At its core, MSTGAT-Net processes the input tensor $\mathbf{X} \in \mathbb{R}^{B \times T \times N}$ (Batch Size $\times$ Time Steps $\times$ Nodes) through three primary, sequential modules, each designed to address a specific aspect of the spatiotemporal forecasting problem:

\begin{enumerate}
    \item \textbf{Temporal Feature Extraction Module (TFEM)}: Transforms raw time series into latent representations by applying depthwise separable convolutions \textit{along the temporal axis}, capturing local and global temporal patterns within each node's history.
    \item \textbf{Low-rank Adaptive Graph Attention Module (LR-AGAM)}: Captures inter-node spatial relationships through a novel efficient adaptive graph attention mechanism with low-rank approximations, learning which regions influence each other.
    \item \textbf{GRU-based Progressive Prediction Refinement Module (PPRM)}: Generates forecasts autoregressively using a GRU, with each prediction step conditioned on the previous, while adaptively blending with learned decay extrapolations from recent observations.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Autoregressive Prediction}: Using GRU to capture dependencies between prediction steps for improved multi-horizon forecasts.
...
The Temporal Feature Extraction Module forms the foundational first stage of MSTGAT-Net, responsible for transforming raw time series data into latent representations that capture meaningful temporal patterns. This module processes input sequences \textit{along the time dimension} using depthwise separable convolutions, extracting both short-term fluctuations and longer-term trends from each node's historical observations.
...
\subsubsection{Depthwise Separable Temporal Convolutions}

Traditional convolutional operations often require prohibitive computational resources when applied to large spatiotemporal data. To address this, we employ depthwise separable convolutions applied \textit{along the time dimension} that decompose the standard convolution into two sequential, more efficient operations:
...
The output of this module, $\mathbf{H}_{reshaped}$, contains rich latent representations of the input time series that encode temporal dynamics while being efficiently structured for subsequent processing by LR-AGAM. Critically, the temporal information from the input window is now compressed into the feature dimension, enabling the subsequent modules to focus on spatial relationships.

\subsection{Low-rank Adaptive Graph Attention Module (LR-AGAM)}
\label{sec:spatial_dependency}

The Low-rank Adaptive Graph Attention Module (LR-AGAM) addresses the challenge of modeling dynamic spatial relationships without relying on predefined structures. After temporal features have been extracted by TFEM, LR-AGAM learns which spatial nodes (regions) influence each other by computing attention weights across the node dimension. Conventional graph neural networks typically operate on fixed adjacency matrices, limiting their ability to adapt to evolving spatial dependencies. Our module employs a novel adaptive graph attention mechanism that dynamically learns and updates spatial relationships during both training and inference.

\subsubsection{Design Principles and Motivation}

Several key principles inform the design of LR-AGAM:
...
Starting with node features $\mathbf{H}_{reshaped} \in \mathbb{R}^{B \times N \times D}$ from TFEM, we implement a memory-efficient attention mechanism using low-rank projections.
...
The key theoretical advantages of LR-AGAM include:
...
The output of this module, $\mathbf{H}_{spatial}$, encodes rich representations that capture spatial dependencies between regions, serving as input to the subsequent Multi-Scale Spatial Feature Module.

\subsection{Multi-Scale Spatial Feature Module (MSSFM)}
\label{sec:multi_scale_spatial}

The Multi-Scale Spatial Feature Module (MSSFM) complements the pairwise attention mechanism of LR-AGAM by capturing multi-scale structural patterns across the spatial graph. While LR-AGAM computes direct node-to-node attention weights, MSSFM applies dilated 1D convolutions \textit{along the node dimension} to aggregate information from neighborhoods of varying sizes. This enables the model to capture both local cluster dynamics (nearby regions with similar patterns) and broader structural relationships (distant but functionally connected regions).

\subsubsection{Dilated Multi-Scale Spatial Convolutions}

After extracting pairwise dependencies with attention, we employ dilated convolutions \textit{across the node axis} to capture multi-scale spatial features. The dilation allows expansion of the receptive field exponentially without increasing parameter count or computational complexity linearly.

Given the spatially-aware node representations $\mathbf{H}_{spatial} \in \mathbb{R}^{B \times N \times D}$ from LR-AGAM, we first transpose the tensor to align the convolution along the node dimension:

\begin{equation}
\mathbf{H}_{spatial}^T \in \mathbb{R}^{B \times D \times N}
\end{equation}

We then apply a series of parallel 1D convolutions with increasing dilation rates to capture patterns at different spatial scales (neighborhoods of different sizes):

\begin{equation}
\mathbf{Z}_s = \text{Conv1D}(\mathbf{H}_{spatial}^T, \text{kernel\_size}=K, \text{dilation}=2^{s-1})
\end{equation}

where $s \in \{1,2,...,S\}$ indexes the scale, and the dilation rate increases exponentially with $s$ (typically, $S=4$ in our implementation). For example:
\begin{itemize}
    \item Scale 1: Dilation = 1, capturing immediate local spatial patterns (receptive field of $K$).
    \item Scale 2: Dilation = 2, capturing medium-range spatial patterns (receptive field of $2K-1$).
    \item Scale 3: Dilation = 4, capturing longer-range spatial patterns (receptive field of $4K-3$).
    \item Scale 4: Dilation = 8, capturing broad spatial patterns (receptive field of $8K-7$).
\end{itemize}

This design allows the model to efficiently cover a large spatial receptive field with few parameters. For instance, with $K=3$ and $S=3$, we achieve a receptive field of 9 nodes using only 9 parameters (3 per scale), compared to 9 parameters required by a standard convolution with kernel size 9.
...
Different regions may exhibit varying spatial characteristics—dense urban clusters might require fine-grained analysis, while sparse rural patterns might be better captured by larger spatial scales. Rather than treating all scales equally, we implement an adaptive fusion mechanism that learns to weigh the importance of different spatial scales:
...
The Multi-Scale Spatial Feature Module draws inspiration from wavelet analysis and multi-resolution processing in signal processing theory. Key benefits include:
...
PPRM takes a hybrid approach, combining the strengths of both methods through a progressive prediction strategy with an adaptive blending mechanism.
...
PPRM offers several theoretical advantages over standard approaches:
...
These characteristics make PPRM particularly well-suited for epidemiological forecasting, where sudden changes in disease dynamics, intervention measures, or reporting practices can cause abrupt shifts in time series behavior.
...
\subsection{MSTGAT-Net without MSSFM (MSTGAT-no-MSSFM)}
\label{sec:ablation_no_mtfm}

This ablation study focuses on the contribution of the multi-scale spatial processing capability. The Multi-Scale Spatial Feature Module (MSSFM), which uses dilated convolutions to capture spatial patterns across different receptive fields with adaptive fusion, is replaced by a single-scale convolution module. The aim is to evaluate how crucial capturing feature interactions at multiple spatial scales simultaneously is for forecasting accuracy compared to using a single-scale approach.
...
\end{equation}
where $\mathbf{H}_s$ is the output from the spatial module, Conv1D performs the single-scale convolution, BN is Batch Normalization, $\sigma$ is ReLU, LN is Layer Normalization, and Fusion represents subsequent linear projection layers. By removing the dilated convolutions and the adaptive fusion across scales, this variant lacks the capacity to efficiently capture multi-scale feature dependencies and adaptively weigh the importance of different spatial receptive fields, which is a key feature of the original MSSFM.

\subsection{MSTGAT-Net without PPRM (MSTGAT-no-PPRM)}
\label{sec:ablation_no_pprm}

This variant assesses the effectiveness of the adaptive refinement mechanism in the final prediction stage. We replace the `HorizonPredictor`, which implements the Progressive Prediction Refinement Module (PPRM), with a simpler `DirectPredictionModule`. The goal is to compare the performance of the progressive approach, which combines model predictions with recent observations via a learned gate, against a straightforward direct multi-step prediction.

The `DirectPredictionModule` utilizes a feed-forward network (typically composed of linear layers, normalization, activations, and dropout) to directly map the final hidden representation $\mathbf{H}_s$ from the spatial feature module to the entire prediction horizon $h$ in a single step:
\begin{equation}
\hat{\mathbf{Y}} = \text{Predictor}(\mathbf{H}_t) \in \text{mathbb{R}}^{B \times N \times h}
\end{equation}
This direct approach contrasts with PPRM, which generates an initial prediction $\mathbf{P}_{initial}$ and then adaptively combines it with a time-decayed extrapolation $\mathbf{E}$ of the last observation using a learned gate $\mathbf{G}$ ($\hat{\mathbf{Y}} = \mathbf{G} \odot \mathbf{P}_{initial} + (1 - \mathbf{G}) \odot \mathbf{E}$). By omitting this gating mechanism and the explicit incorporation of the most recent observation for refinement, the `DirectPredictionModule` variant may be less adaptive to sudden changes or shifts in the time series dynamics compared to the progressive approach employed in the full MSTGAT-Net model.

By comparing the performance metrics (e.g., MAE, RMSE, MAPE) of the full MSTGAT-Net model against these three ablated variants (MSTGAT-no-AGAM, MSTGAT-no-MSSFM, MSTGAT-no-PPRM) on benchmark datasets, we can quantitatively determine the specific contributions of LR-AGAM (adaptive graph attention), MSSFM (multi-scale spatial feature fusion), and PPRM (progressive prediction refinement) to the overall forecasting accuracy and robustness of the proposed architecture.
...
\begin{table}[ht]
\centering
\caption{Ablation study results for 5-day forecasting. MSTGAT-no-AGAM removes LR-AGAM (Low-rank Adaptive Graph Attention Module), MSTGAT-no-MSSFM removes MSSFM (Multi-Scale Spatial Feature Module), and MSTGAT-no-PPRM removes PPRM (Progressive Prediction Refinement Module). Best results are in \textbf{bold}.}
...
MSTGAT-no-MSSFM & 7.25 & 12.04 & 25.8\% & 8.93 & 14.52 & 31.5\% \\
MSTGAT-no-PPRM & 7.02 & 11.59 & 24.9\% & 8.76 & 14.23 & 30.9\% \\
\midrule
...
\caption{Component importance comparison across different forecast horizons. The bars represent the percentage increase in RMSE when each component is removed, indicating their relative contribution to model performance. Note how LR-AGAM (Low-rank Adaptive Graph Attention Module) becomes increasingly important for longer forecast horizons.}
...
\begin{itemize}
    \item LR-AGAM (Low-rank Adaptive Graph Attention Module) contributes most significantly to model performance, with its removal leading to a 9.8\% increase in MAE on the Japan dataset and an 8.9\% increase on the Regional dataset.
    \item MSSFM (Multi-Scale Spatial Feature Module) provides the second-largest contribution, demonstrating the importance of capturing spatial feature interactions at different scales with adaptive fusion.
    \item PPRM (Progressive Prediction Refinement Module) offers a smaller but still noticeable improvement, particularly for the 5-day horizon shown here. Further analysis showed that PPRM's contribution becomes more significant for longer horizons (10-day, 15-day), highlighting its role in mitigating error accumulation through adaptive blending.
...
Fig. \ref{fig:attention_patterns} illustrates the learned attention patterns from LR-AGAM on the Japan COVID-19 dataset. The heatmap reveals several interesting spatiotemporal patterns:
...
\subfloat[Without LR-AGAM]{\includegraphics[width=0.33\textwidth]{/home/olarinoyem/Project/MSAGAT-Net/report/figures/matrices_MSTAGAT-Net.japan.w-20.h-3.no_agam.png}}
\subfloat[Without MSSFM]{\includegraphics[width=0.33\textwidth]{/home/olarinoyem/Project/MSAGAT-Net/report/figures/matrices_MSTAGAT-Net.japan.w-20.h-3.no_mtfm.png}}
\caption{Learned attention patterns (adjacency matrices) for Japan COVID-19 dataset. (a) Full MSTGAT-Net model shows clear, structured patterns that align with geographical adjacency and population connectivity. (b) Removing LR-AGAM results in less defined spatial relationships. (c) Removing MSSFM affects how the model captures multi-scale spatial patterns, which impacts the learned attention distribution.}
...
\subsubsection{Analysis of Multi-Scale Spatial Features}

Fig. \ref{fig:scale_weights} shows the learned scale weights from the adaptive fusion mechanism in the Multi-Scale Spatial Feature Module. The analysis reveals that:
...
\begin{itemize}
    \item PPRM's (Progressive Prediction Refinement Module) adaptive blending of model predictions with recent observations.
    \item MSSFM's (Multi-Scale Spatial Feature Module) ability to capture patterns at different spatial scales simultaneously through adaptive fusion.
    \item The dynamic nature of LR-AGAM (Low-rank Adaptive Graph Attention Module), which can rapidly adjust to changes in inter-regional transmission patterns.
\end{itemize}
...
This paper introduced MSTGAT-Net, a novel deep learning architecture for spatiotemporal forecasting that combines adaptive graph attention, multi-scale spatial feature modeling, and progressive prediction. Through comprehensive evaluations on multiple real-world epidemic datasets, we demonstrated that MSTGAT-Net consistently outperforms state-of-the-art baselines across various forecast horizons, with particularly strong performance for medium and long-term predictions.
...
Our ablation studies (Fig. \ref{fig:component_importance} and Fig. \ref{fig:component_importance_heatmap}) quantified the contributions of each key architectural innovation, revealing that the Adaptive Graph Attention Module provides the largest performance improvement (9.8\% increase in MAE when removed), followed by the Multi-Scale Spatial Feature Module (6.5\%) and the Progressive Prediction Module (3.1\%). The visualization of learned attention patterns (Fig. \ref{fig:attention_patterns}) showed that the model captures meaningful spatial relationships that align with geographical proximity and population connectivity, despite not being explicitly provided with this information.
