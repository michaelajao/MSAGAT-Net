...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}[1]
    \REQUIRE 
        Dataset $\{X,Y\}$, optional adjacency $A$, hyperparams $(\eta,\lambda,p,\gamma,E)$  
    \ENSURE 
    ...
    \FOR{$e=1,2,\dots,E$}  
      ...
      \IF{$L_{\rm val}<L_{\rm val}^*$}  
        ...
        \STATE $\mathrm{Eval}(\mathrm{test},\Theta^*)$  
      \ELSE  
...
\begin{algorithm}[h]
  \caption{MSAGATNet: Mathematical Formulation}
  \label{alg:msagatnet_math}
  \begin{algorithmic}[1]
    \REQUIRE 
      $X\in\mathbb{R}^{B\times T\times N}$,\quad optional $A_0\in\mathbb{R}^{N\times N}$
    \ENSURE 
      $\hat{Y}\in\mathbb{R}^{B\times h\times N},\;L_{\rm att}$
      
    \STATE $\tilde{X}\!=\!{\rm reshape}(X)\in\mathbb{R}^{BN\times1\times T}$ \hfill\COMMENT{Reshape for temporal convolution}
    
    \STATE $Z_{\rm dep}=\mathrm{DWConv}(\tilde{X})$,\ 
           $Z=\mathrm{PWConv}(\sigma\,{\rm BN}(Z_{\rm dep}))$ \hfill\COMMENT{Depthwise separable convolution}
           
    \STATE $H=\sigma\bigl({\rm LN}(ZW_L W_H+b)\bigr)\in\mathbb{R}^{BN\times D}$ \hfill\COMMENT{Low-rank feature processing}
    
    \STATE reshape $H\to H^{(B,N,D)}$, \ 
           $X_L=X_{:,-1,:}\in\mathbb{R}^{B\times N}$ \hfill\COMMENT{Extract last observation}
           
    \STATE $[Q,K,V]=\mathrm{split}(H\,W_{qkv}^L\,W_{qkv}^H+b_{qkv})$ \hfill\COMMENT{Low-rank QKV projection}
    
    \STATE $\widetilde{Q}=\mathrm{ELU}(Q)+1,\;\widetilde{K}=\mathrm{ELU}(K)+1$ \hfill\COMMENT{Positive mapping for linearization}
    
    \STATE $A=\mathrm{softmax}\bigl(\tfrac{\widetilde{Q}\widetilde{K}^T}{\sqrt{D}}+UV^T+\lambda A_0\bigr)$ \hfill\COMMENT{Attention with factorized adjacency}
    ...
    \STATE $L_{\rm att}=\lambda_{\rm att}\|A\|_1$ \hfill\COMMENT{Attention regularization loss}
    ...
    \STATE $P=\mathrm{pred}(H_t),\quad G=\mathrm{sigmoid}(\mathrm{gate}(H_t))$ \hfill\COMMENT{Initial prediction and gate}
    
    \STATE $E=X_L\odot\exp(-\beta\,[1:h])$ \hfill\COMMENT{Time-decayed last observation}
    
    \STATE $\hat{Y}=G\odot P+(1-G)\odot E$ \hfill\COMMENT{Progressive prediction with gating}
  \end{algorithmic}
\end{algorithm}
...
\end{tikzpicture}
\caption{Diagram of the Feature Extraction Module}
