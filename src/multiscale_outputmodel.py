...
class MultiScalePredictionModule(nn.Module):
    """
    Multi-Scale Prediction Module for multi-step forecasting.
    
    Uses multiple parallel prediction pathways at different scales,
    with adaptive fusion to combine predictions optimally.
    """

    def __init__(self, hidden_dim, horizon, num_scales=3, dropout=DROPOUT):
        super(MultiScalePredictionModule, self).__init__()
        self.horizon = horizon
        self.num_scales = num_scales
        
        # Create predictors for different scales
        ...
        # Learnable fusion weights - one weight per scale and time step
        self.fusion_weights = nn.Parameter(torch.ones(num_scales, horizon), requires_grad=True)
        
    def forward(self, x, last_step=None):
        """
        Forward pass of the multi-scale prediction module.
        ...
        scale_preds = []
        for predictor in self.scale_predictors:
            pred = predictor(x)  # [B, N, horizon]
            scale_preds.append(pred)
        
        # Stack predictions
        stacked = torch.stack(scale_preds, dim=1)  # [B, num_scales, N, horizon]
        
        # Compute scale fusion weights
        weights = F.softmax(self.fusion_weights, dim=0)  # [num_scales, horizon]
        weights = weights.view(1, self.num_scales, 1, self.horizon)
        
        # Combine predictions with weights
        fused_preds = (stacked * weights).sum(dim=1)  # [B, N, horizon]
...
class MSAGATNet(nn.Module):
    ...
    4. Multi-Scale Prediction Module for scale-specific forecasting
    """
    ...
    def __init__(self, args, data):
        ...
        # Using the new Multi-Scale Prediction Module
        self.prediction_module = MultiScalePredictionModule(
            self.hidden_dim, self.horizon,
            num_scales=min(3, getattr(args, 'num_scales', NUM_SCALES)),  # Use 3 or fewer scales
            dropout=getattr(args, 'dropout', DROPOUT)
        )
    ...
    def forward(self, x, idx=None):
        ...
        # Generate predictions at multiple scales (no need for last observed value)
