"""
MAGATFN - Original/Simpler Model Variant

This module contains the MAGATFN model which is a simpler variant of MSTAGAT-Net.
The key difference is that MAGATFN does NOT use the learned graph bias in the 
forward pass - it only uses it for regularization. This can be beneficial for
large graphs (>40 nodes) where learned attention alone performs better.

IMPORTANT: For new experiments, prefer using models.py with --no_graph_bias flag
instead of this file. This file is kept for reference and reproducibility.

To get MAGATFN-like behavior with models.py:
    python -m src.scripts.train --model msagat --no_graph_bias ...

Key Differences from MSTAGAT-Net (models.py):
- Graph bias used for REGULARIZATION ONLY (not added to forward pass output)
- Original defaults: dropout=0.25, num_scales=4 (vs 0.20 and 3 in paper)
"""
...
# NOTE: These differ from paper defaults in models.py (dropout=0.20, num_scales=3)
HIDDEN_DIM = 32
ATTENTION_HEADS = 4
# Initial value for adaptive attention regularization weight
ATTENTION_REG_WEIGHT_INIT = 1e-05
DROPOUT = 0.25  # Paper uses 0.20
NUM_TEMPORAL_SCALES = 4  # Paper uses 3
KERNEL_SIZE = 3
FEATURE_CHANNELS = 16
BOTTLENECK_DIM = 8
